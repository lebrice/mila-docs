...
from deepspeed.ops.op_builder.transformer import TransformerBuilder
from deepspeed.ops.op_builder.stochastic_transformer import StochasticTransformerBuilder
...
class DeepSpeedTransformerConfig(TransformerConfig):
    ...
    intermediate_size: int
    heads: int
    attn_dropout_ratio: int | float
    hidden_dropout_ratio: int | float
    num_hidden_layers: int
    initializer_range: float
    local_rank: int = -1
    ...
    gelu_checkpoint: bool = False  # True: if higher batch size is required
    ...
    @classmethod
    def from_json_file(cls, json_file: str | Path):
...
class DeepSpeedTransformerFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        input: Tensor,
        input_mask: Tensor,
        self: DeepSpeedTransformerLayer,
        grads: list[tuple[Tensor, str]] | None,
        layer_id: int,
        attn_qkvw: nn.Parameter | Tensor,
        attn_qkvb: nn.Parameter | Tensor,
        attn_ow: nn.Parameter | Tensor,
        attn_ob: nn.Parameter | Tensor,
        attn_nw: nn.Parameter | Tensor,
        attn_nb: nn.Parameter | Tensor,
        inter_w: nn.Parameter | Tensor,
        inter_b: nn.Parameter | Tensor,
        output_w: nn.Parameter | Tensor,
        output_b: nn.Parameter | Tensor,
        norm_w: nn.Parameter | Tensor,
        norm_b: nn.Parameter | Tensor,
        config: DeepSpeedTransformerConfig,
    ):
        cuda_module = (
        ...
        if grads is not None:
            ...
            #         )
            #     )
            def weird_hook(x: Tensor, i=2, suffix: str = "W"):
                grads.append(
                    (
                        x[i * attn_ow.size(0) : (i + 1) * attn_ow.size(0)],
                        (f"Q_{suffix}" if i == 0 else f"K_{suffix}" if i == 1 else f"V_{suffix}"),
                    )
                )

            def append_to_grads(x: Tensor, name: str):
                grads.append((x, name))

            attn_qkvw.register_hook(functools.partial(weird_hook, suffix="W"))
            attn_qkvb.register_hook(functools.partial(weird_hook, suffix="B"))
            attn_ow.register_hook(functools.partial(append_to_grads, name="O_W"))
            attn_ob.register_hook(functools.partial(append_to_grads, name="O_B"))
            attn_nw.register_hook(functools.partial(append_to_grads, name="N2_W"))
            attn_nb.register_hook(functools.partial(append_to_grads, name="N2_B"))
            inter_w.register_hook(functools.partial(append_to_grads, name="int_W"))
            inter_b.register_hook(functools.partial(append_to_grads, name="int_B"))
            output_w.register_hook(functools.partial(append_to_grads, name="out_W"))
            output_b.register_hook(functools.partial(append_to_grads, name="out_B"))
            norm_w.register_hook(functools.partial(append_to_grads, name="norm_W"))
            norm_b.register_hook(functools.partial(append_to_grads, name="norm_B"))
    ...
    def backward(ctx, grad_output):
        ...
        if grad_output_shape[1] % 16 != 0:
            grad_output = torch.cat(
                (
                    grad_output,
                    torch.zeros(
                        (bsz, (16 - (grad_output_shape[1] % 16)), grad_output_shape[2]),
        ...
        (
            ...
            (
                ctx.inp_norm
                if (ctx.config.pre_layer_norm or not ctx.config.normalize_invertible)
                else input
            ),
            ctx.qkv_tf,
            ctx.soft_inp,
...
class InitialWeightsTuple(NamedTuple):
    q: Tensor
    k: Tensor
    v: Tensor
...
class InitialBiasesTuple(NamedTuple):
    unused1: Any
    unused2: Any
    unused3: Any
...
class DeepSpeedTransformerLayer(nn.Module):
    ...
    def __init__(
        self,
        config: DeepSpeedTransformerConfig,
        initial_weights: InitialWeightsTuple | None = None,
        initial_biases: InitialBiasesTuple | None = None,
    ):
        super().__init__()
        self.config = config
        ...
        if initial_weights is None and initial_biases is None:
            ...
            self.attn_qkvw = nn.Parameter(torch.Tensor(h * 3, h))
            self.attn_qkvb = nn.Parameter(torch.Tensor(h * 3))
            self.attn_ow = nn.Parameter(torch.Tensor(h, h))
            self.attn_ob = nn.Parameter(torch.Tensor(h))
            self.attn_nw = nn.Parameter(torch.Tensor(h))
            self.attn_nb = nn.Parameter(torch.Tensor(h))
            self.inter_w = nn.Parameter(torch.Tensor(self.config.intermediate_size, h))
            self.inter_b = nn.Parameter(torch.Tensor(self.config.intermediate_size))
            self.output_w = nn.Parameter(torch.Tensor(h, self.config.intermediate_size))
            self.output_b = nn.Parameter(torch.Tensor(h))
            self.norm_w = nn.Parameter(torch.Tensor(h))
            self.norm_b = nn.Parameter(torch.Tensor(h))
            self.init_transformer_weights(self.config.adjust_init_range)
        else:
            assert initial_weights is not None
            assert initial_biases is not None
            # For testing only.
    ...
    def init_transformer_weights(self, adjust_init_range: bool = False):
